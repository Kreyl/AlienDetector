...
#define RstHi()  { PinSetHi(LCD_RESET_PIN); }
#define RstLo()  { PinSetLo(LCD_RESET_PIN); }
#define CsHi()   { PinSetHi(LCD_CSX_PIN);   }
#define CsLo()   { PinSetLo(LCD_CSX_PIN);   }
#define DcHi()   { PinSetHi(LCD_DC_PIN); }
#define DcLo()   { PinSetLo(LCD_DC_PIN); }
#define WrHi()   { PinSetHi(LCD_WR_PIN); }
#define WrLo()   { PinSetLo(LCD_WR_PIN); }
#define RdHi()   { PinSetHi(LCD_RD_PIN); }
#define RdLo()   { PinSetLo(LCD_RD_PIN); }
#define Write(Value) PortSetValue(LCD_DATA_GPIO, Value)
#endif

void ILI9341_t::Init() {
    // ==== GPIO ====
    PinSetupOut(LCD_RESET_PIN, omPushPull);
    PinSetupOut(LCD_CSX_PIN,   omPushPull);
    PinSetupOut(LCD_DC_PIN,    omPushPull, psHigh);
    PinSetupOut(LCD_WR_PIN,    omPushPull, psHigh);
    PinSetupOut(LCD_RD_PIN,   omPushPull, psHigh);
    // Data port
